...
def render_draw_view():
    ...
    with c2:
        scat_select = st.selectbox(
            "Subcategoría", ["AA", "A", "B", "B+", "C", "D", "Mini-Tenis", "8-10 años"]
        )
    with c3:
    ...
    last_f = (
        st.session_state.current_draw.get("filters")
    ...
    if len(eligible_df) == 0:
        st.info(
            f"No hay jugadores registrados en {cat_select} - {scat_select} ({format_select})."
        )
        return
    ...
    if format_select == "Dobles":
        ...
        pago_dict = {
            get_full_name(r): str(r.get("Pago", "Pendiente")).strip()
        ...
        eligible_df = eligible_df.drop_duplicates(subset=["pair_key"]).drop(
            columns=["pair_key"]
        )
    ...
    with c4:
        ...
        lbl_text = (
            "**Total de Parejas Elegibles:**"
            if format_select == "Dobles"
            else "**Total de Jugadores Elegibles:**"
        )
        st.info(f"{lbl_text} {total_players}")
    with c5:
        default_groups = max(1, total_players // 3)
        num_groups = st.number_input(
            "Cantidad de Grupos (M)",
            min_value=1,
    ...
    st.caption(
        f"Selecciona hasta {num_groups} jugadores manuales que liderarán cada grupo."
    )
    ...
    if format_select == "Dobles":
        ...
        col_cfg = {
            "Seleccionar Seed": st.column_config.CheckboxColumn(
                "Cabeza de Serie",
                help="Selecciona los cabezas de serie",
                default=False,
                width="small",
            ),
    ...
    if num_seeds > num_groups:
        st.error(
            f"¡Atención! Has seleccionado {num_seeds} seeds, pero solo hay {num_groups} grupos. Desmarca {num_seeds - num_groups}."
        )
        valid_seeds = False
    else:
    ...
    with btn_col1:
        if st.button(
            "🎲 SORTEAR Y CREAR GRUPOS (DRAFT)",
            use_container_width=True,
            disabled=not valid_seeds or is_finalized,
        ):
    ...
    with btn_col2:
        can_save = (
            st.session_state.current_draw.get("is_dirty")
            if st.session_state.current_draw
            else False
        )
        if st.button(
            ...
            type="primary",
            use_container_width=True,
            disabled=not can_save or is_finalized,
        ):
            from utils.supabase_data import save_tournament_draw

            draw_data = st.session_state.current_draw
            if save_tournament_draw(
                t_id, cat_select, scat_select, format_select, draw_data["groups"]
            ):
    ...
    with st.expander("VISTA PREVIA DE GRUPOS", expanded=has_draw):
        if not has_draw:
            st.info(
                "Configura la lógica y haz clic en 'Sortear' para visualizar los grupos."
            )
        else:
            draw_data = st.session_state.current_draw
            ...
            st.markdown(
                f"#### Resultados: {f_cat} - {f_scat} ({f_fmt}) <small style='opacity:0.5;'>{status_tag}</small>",
            ...
            for idx, (g_name, players) in enumerate(draw_data["groups"].items()):
                with cols[idx % 2]:
                    st.markdown(
                        f"<h5 style='color: #ffffff; text-shadow: 0 0 10px rgba(255,255,255,0.4);'>{g_name}</h5>",
                        unsafe_allow_html=True,
                    )
                    for p in players:
                        seed_badge = (
                            "<strong style='color:#CCFF00; font-size:0.75rem; font-family: \"Montserrat\", sans-serif; font-style: italic; font-weight: bold; margin-right: 4px;'>SEED  </strong>"
                            if p.get("is_seed")
                            else ""
                        )
